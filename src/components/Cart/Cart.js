...
const Cart = props => {
    ...
    const fetchCart = useCallback(async () => {
        try {
            ...
            const response = await fetch(`https://crudcrud.com/api/7913bbf556a942cdb164241232b29434/cart`);
    ...
    carts.forEach((item) => {
        if (map.has(item.id)) {
            ...
            map.set(item.id, {
                id: it.id, tshirtname: it.tshirtname, description: it.description, price: it.price,
